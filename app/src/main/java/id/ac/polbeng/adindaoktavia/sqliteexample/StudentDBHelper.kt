...
class StudentDBHelper(context: Context) : SQLiteOpenHelper(
    ...
    fun searchStudentByNIM(nim: String): ArrayList<StudentModel> {
        ...
        try {
            cursor = db.rawQuery(
                "select * from " + DBContract.UserEntry.TABLE_NAME + " WHERE " +
    ...
    fun searchStudentByName(name: String): ArrayList<StudentModel> {
        ...
        with(cursor) {
            while (moveToNext()) {
                ...
                studentName = cursor.getString(cursor.getColumnIndexOrThrow(DBContract.UserEntry.COLUMN_NAME))
                age = cursor.getString(cursor.getColumnIndexOrThrow(DBContract.UserEntry.COLUMN_AGE))
    ...
    fun readStudents(): ArrayList<StudentModel> {
        ...
        val cursor: Cursor?
        try {
            cursor = db.rawQuery("select * from " + DBContract.UserEntry.TABLE_NAME + " order by " +
